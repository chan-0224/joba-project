...
def get_current_user(
    ...
    payload = decode_token(token)  # exp/서명 검증
...
def kakao_callback(code: str, db: Session = Depends(get_db)):
    ...
    if not pid:
        raise HTTPException(400, "카카오 사용자 id를 가져오지 못했습니다.")
...
def naver_callback(code: str, state: str, db: Session = Depends(get_db)):
    ...
    if not pid:
        raise HTTPException(400, "네이버 사용자 id를 가져오지 못했습니다.")
...
def google_callback(code: str, db: Session = Depends(get_db)):
    ...
    if not pid:
        raise HTTPException(400, "구글 사용자 id를 가져오지 못했습니다.")
